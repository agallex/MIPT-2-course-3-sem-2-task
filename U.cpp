...
void dfs (Graph_list& G, Vertex v, std::vector<int64_t>& used, std::set<Vertex>& component, int64_t timer, std::vector<int64_t>& tin, std::vector<int64_t>& fup, int64_t p = -1) {
    ...
    for (auto elem: neighbors) {
        ...
        if (used[elem] == 1) {
            ...
            dfs(G, elem, used, component, timer, tin, fup, v);
...
std::set<Vertex> points_of_articulation(Graph_list& G, Vertex n) {
    ...
    for (Vertex i = 0; i < G.Get_vertex_size(); ++i) {
        if (used[i] == 0) {
            int64_t timer = 0;
            dfs(G, i, used, component, timer, tin, fup);
        }
    }

    for (auto elem: component) {
        if (elem >= n) {
            answer.insert(elem - n);
        }
    }
...
int main() {
    ...
    Vertex not_vertex = n + 1;

    for (int64_t i = 0; i < m; ++i) {
        Vertex first, second, third;
        std::cin >> first >> second >> third;
        graph.Add_edge(first, not_vertex);
        graph.Add_edge(second, not_vertex);
        graph.Add_edge(third, not_vertex);
        ++not_vertex;
    }

    auto answer = points_of_articulation(graph, n);
